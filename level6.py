...
class Level(pg.sprite.Sprite):
    ...
    def set_role(self):
        ...
        self.enemy2 = enemy2()
        self.enemy2.pos = vec(WIDTH * 0.7, GROUND_HEIGHT - 70)  # 初始位置（屏幕中央偏上）
        self.all_role = pg.sprite.Group(self.mario, self.enemy2)
    ...
    def check_collide(self):
        """检测马里奥与所有线段碰撞体的碰撞"""
        # 分别检测水平线和垂直线的碰撞
        for role in self.all_role:
            self.horizontal_collisions = pg.sprite.spritecollide(role, self.horizontal_lines, False)
            self.vertical_collisions = pg.sprite.spritecollide(role, self.vertical_lines, False)
        ...
        for role in self.all_role:
            for line in self.horizontal_collisions:
                # 检查是否站在水平线上
                if (role.vel.y >= 0 and 
                    abs(role.rect.bottom - line.rect.top) < 5):
                    self.on_ground = True
    ...
    def adjust_horizontal_collisions(self):
        ...
        for role in self.all_role:
            for line in self.horizontal_collisions:
                # 从上方落到水平线上
                if (role.vel.y > 0 and 
                    role.rect.bottom > line.rect.top and
                    role.rect.bottom - role.vel.y <= line.rect.top):
                    
                    role.acc.y = 0        # 重置垂直加速度
                    role.vel.y = 0        # 重置垂直速度
                    role.pos.y = line.rect.top  # 放置在线段顶部
                    role.landing = True   # 标记为着陆状态
                    found_ground = True
                    break  # 只需要处理一个顶部碰撞
                    
                # 从下方碰撞水平线底部
                elif (role.vel.y < 0 and 
                    role.rect.top < line.rect.bottom and
                    role.rect.top - role.vel.y >= line.rect.bottom):
                    
                    role.vel.y = 0        # 重置垂直速度
                    role.pos.y = line.rect.bottom + role.rect.height
    ...
    def check_dead(self):
        """检查马里奥是否死亡（掉出地图底部）"""
        for role in self.all_role:
            if role.pos.y > GROUND_HEIGHT + 50:  # 如果掉到地面以下一定距离
                role.dead = True  # 标记为死亡状态
            # 同时检查是否走出地图右边界
            if role.pos.x > MAP_WIDTH + 20:  # 如果走出地图右边界
                role.dead = True  # 也可以标记为完成关卡，这里简单处理为死亡
        ...
        for line in self.vertical_collisions:
            ...
            # 如果马里奥站在与这条垂直线相连的水平线上，允许他移动通过
                if standing_on_connected_horizontal:
                    # 只有当马里奥试图穿过垂直线时才阻止他
                    if (role.vel.x > 0 and 
                        role.rect.right > line.rect.left and
                        role.rect.right - role.vel.x <= line.rect.left):
                        # 向右移动时碰到垂直线
                        role.pos.x = line.rect.left - role.rect.width/2
                        role.vel.x = 0
                    elif (role.vel.x < 0 and 
                        role.rect.left < line.rect.right and
                        role.rect.left - role.vel.x >= line.rect.right):
                        # 向左移动时碰到垂直线
                        role.pos.x = line.rect.right + role.rect.width/2
                        role.vel.x = 0
                else:
                    # 标准水平碰撞处理
                    if role.vel.x > 0:  # 向右移动时碰撞
                        role.pos.x = line.rect.left - role.rect.width/2
                        role.vel.x = 0
                    elif role.vel.x < 0:  # 向左移动时碰撞
                        role.pos.x = line.rect.right + role.rect.width/2
                        role.vel.x = 0