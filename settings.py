...
MAP_WIDTH = 4*800  # 地图宽度，设置为窗口宽度的两倍
...
JUMP = 20          # 跳跃力：控制马里奥跳跃的高度:向上有速度,速度为-值,向下有速度,速度为正值
TURNAROUND = 0.5   # 转向加速度：改变方向时的加速度
MAX_SPEED = 4.5      # 最大速度：限制马里奥水平移动的最高速度
TERMINAL_VELOCITY=9.9   # 最大速度：限制马里奥垂直移动的最高速度

# 地面高度
GROUND_HEIGHT = HEIGHT - 66 +30 # 地面在屏幕上的y坐标（窗口高度减去66像素）:地面和窗口最下端之间的东西
PLAIN_HEIGHT = 20  # 平台的高度（像素）:从地面的最下端到地面的最上端
#height越大代表越向下
