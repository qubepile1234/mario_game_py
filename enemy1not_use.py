...
class Enemy1(EnemyBase):
    ...
    def load_from_sheet(self):
        ...
        # 使用50%宽度，100%高度（示例，你可以根据需要调整）
        self.right_frames.append(self.get_image(0, 0, 20, 20, 50, 100))  # 站立/移动帧1
        self.right_frames.append(self.get_image(40, 0, 20, 20, 50, 100)) # 移动帧2
        self.right_frames.append(self.get_image(60, 0, 20, 20, 50, 100)) # 移动帧3
        
        # 跳跃/下落帧（使用不同的缩放）
        self.right_frames.append(self.get_image(90, 0, 16, 16, 50, 100))
...
class Enemy2(EnemyBase):
    ...
    def load_from_sheet(self):
        ...
        # 加载向右动画帧（从精灵图中截取特定区域）
        # 使用不同的缩放参数：120%宽度，120%高度（放大20%）
        self.right_frames.append(
            self.get_image(178, 96, 12, 16, 80, 80))  # 站立帧
        self.right_frames.append(
            self.get_image(80, 96, 15, 16, 80, 80))   # 行走帧1
        self.right_frames.append(
            self.get_image(96, 96, 16, 16, 80, 80))   # 行走帧2
        self.right_frames.append(
            self.get_image(112, 96, 16, 16, 80, 80))  # 行走帧3
        self.right_frames.append(
            self.get_image(144, 96, 16, 16, 80, 80))  # 跳跃帧
